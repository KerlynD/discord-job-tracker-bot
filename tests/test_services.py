...
from models import Base, Application, Stage, Reminder
...
class TestJobTrackerService:
    ...
    def test_list_applications(self, service):
        ...
        app2 = service.add_application("Meta", "Product Manager", 123)
    ...
    def test_get_stale_applications(self, service):
        ...
        for stage in all_stages:
            if stage.stage == "Applied":
                ...
                stage.date = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=10)
            elif stage.stage == "OA":
                # Make OA stage stale but more recent than Applied
                stage.date = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=8)
        service.db.commit()
    ...
    def test_get_due_reminders(self, service):
        ...
        past_date = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=1)
    ...
    def test_get_application_stats(self, service):
        ...
        app1 = service.add_application("Google", "Software Engineer", 123)
        app2 = service.add_application("Meta", "Product Manager", 123)
        app3 = service.add_application("Amazon", "DevOps Engineer", 123)
